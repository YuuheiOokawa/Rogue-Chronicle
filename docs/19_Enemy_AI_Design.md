...
- ※本表（10/30/50/100）が確定値。16章§9.1の表も本表に合わせて改訂済み（ISSUE-112解消）
...
| ID | 内容 |
|---|---|
| ISSUE-111 | 解消済み（16章を修正済み）: ボスのフェーズ3閾値の不一致（本書40%=HP199以下 / 16章§8旧35%）。**本書40%を正**として16章§8を改訂した |
| ISSUE-112 | 解消済み（16章を修正済み）: ドロップ発生率の不一致（本書 通常10%/強敵30%/エリート50%/ボス100% / 16章§9.1旧30/50/100/100）。**本書を正**として16章§9.1を改訂した。実プレイでの装備入手テンポは引き続き要検証 |
