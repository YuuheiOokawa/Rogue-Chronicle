...
|---|---|---|
| フェーズ1「静観」 | HP > 70%（350超） | — |
| フェーズ2「覚醒」 | 70% ≥ HP > 40%（350〜199超） | 移行した瞬間、intentを**「崩落の一撃」予告**に強制上書き（1ターン予告→次の自行動で発動） |
| フェーズ3「暴走」 | HP ≤ 40%（199以下） | 同上（再度「崩落の一撃」予告）。以降、行動テーブルが攻撃偏重に |
...
| 敵種別 | ドロップ発生率 | 参照テーブル例 |
|---|---|---|
| 通常 | 10% | rt_battle_normal（消耗品60/装備common35/装備rare5） |
| 強敵 | 30% | rt_battle_strong（装備common50/装備rare40/消耗品10） |
| エリート | 50% | rt_battle_elite（装備rare60/装備epic20/レリック20） |
| ボス | 100%（固定+抽選） | rt_boss（レリック1個確定 + 装備epic抽選50%） |

- ドロップ発生率・抽選詳細は19_Enemy_AI_Design.md §9.3（DEC-052）と共通（本表は19章の確定値に合わせて改訂済み）
