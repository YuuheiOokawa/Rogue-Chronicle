...
## DEC-025: リタイアはphaseを問わず即時'retired'（80%）に統一（矛盾解消）
- 状態: Accepted / 決定日: 2026-07-13
- 背景: docs/09 SCR-314は「戦闘中のリタイアは敗北扱い」と記載していたが、docs/17 §7（状態遷移表）は「リタイア: phaseは問わず即時確定、獲得ソウルシャード80%」としており矛盾していた。Phase 7（報酬・スキル・装備）実装にあたり確定させる必要が生じた。
- 決定: docs/17を正とする。リタイアは現在のphase（map_select/battle/reward_pending）にかかわらず常にstatus='retired'（係数80%）へ遷移する。戦闘中でも敗北(50%)扱いにはしない。
- 理由: 実装の単純化（phase分岐が不要）。プレイヤー視点でも「途中でやめる」操作が常に一定の対価で報われる方が体験として分かりやすく、ローグライトの「安全に撤退する」戦略性を損なわない。
- 影響範囲: API-306（retire route）、docs/09 SCR-314の記述をこの決定に合わせて修正。
